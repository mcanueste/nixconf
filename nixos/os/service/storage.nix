{
  ...
  options.nixconf.service.storage = {
    trim = lib.mkOption {
      type = lib.types.bool;
      default = false;
  ...
  config = {
    services = {
      ...
      fstrim.enable = config.nixconf.service.storage.trim;

      # Hard disk protection if the laptop falls:
      hdapsd.enable = config.nixconf.service.storage.hdapsd;
      ...
      gvfs.enable = config.nixconf.service.storage.gvfs;

      # In case gvfs doesn't work, enable udisk2 as well
      udisks2.enable = config.nixconf.service.storage.udisk2;
    };
  };
}
